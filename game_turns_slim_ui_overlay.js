...
function renderPlayersList(){
  ...
  state.players.forEach((p,idx)=>{
    const row=el('div','player'+(p.id===uid?' you':'')+(idx===state.turn?.i?' turn':''));
...
function renderStatus(){
  ...
  const turnIndex = state.turn?.i;
  const turn = (typeof turnIndex==='number') ? state.players[turnIndex] : null;
...
function renderControls(){
  const show = (state.phase==='bidding' && amCurrentPlayer());
  $('#controls').classList.toggle('hidden', !show);
  const calzaBtn=$('#btnCalza'); if(calzaBtn){ calzaBtn.disabled = state.turn?.palifico; calzaBtn.classList.toggle('hidden', state.turn?.palifico); }
  setFirstBidControls();
}
...
async async     else { loser=bidder; reason=`Dudo success: ${total} < ${state.bid.qty}.`; }
...
function processInput(a){
  if(!isHost) return;
  const next = Turns.reduce(state, a);
...
function attachRoomListeners(){
  ...
  stateRef.on('value', snap=>{
    ...
    if(g.status==='playing' || g.players?.length){
      goToGame();
      render();
      if(g.phase==='reveal'){ showRevealOverlay(g.reveal); } else { hideRevealOverlay(); }
      if(g.phase==='gameover'){ showGameOver(g.message||'Game over'); }
    }
  });
...
document.getElementById('btnBid').addEventListener('click', ()=>{
  ...
  inputsRef.push({type:'bid', qty:q, face:f, by:uid, ts: Date.now()});
}); }
}); }
});

document.getElementById('btnDudo').addEventListener('click', ()=>{
  inputsRef.push({type:'dudo', by:uid, ts: Date.now()});
...
document.getElementById('btnCalza').addEventListener('click', ()=>{
  inputsRef.push({type:'calza', by:uid, ts: Date.now()});
...
document.getElementById('btnRevealOk').addEventListener('click', ()=>{
  inputsRef.push({type:'continueReveal', by:uid, ts: Date.now()});
}); }
});
