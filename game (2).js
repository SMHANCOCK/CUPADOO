...
document.getElementById('btnBid').addEventListener('click', ()=>{
  ...
  try{ inputsRef.push({type:'bid', by:uid, qty:q, face:f, ts: Date.now()}); }catch(e){ console.warn(e); }
...
document.getElementById('btnRevealOk').addEventListener('click', ()=>{
  ...
  if(isHost){ try{ inputsRef.push({type:'continueReveal', by:uid, ts: Date.now()}); }catch(e){} }
...
// Initialize host-authoritative turn engine if present
try{ if(window.Turns && typeof window.Turns.init==='function'){ window.Turns.init(); } }catch(e){}
