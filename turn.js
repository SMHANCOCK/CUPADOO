...
case 'bid':{
  ...
  // Ensure we have a valid turn; if not, repair from starter or first alive
  if(!s.turn || !s.turn.id){
    let i = (s.starter && Number.isInteger(s.starter.i)) ? s.starter.i : 0;
    if(!aliveAt(s, i)){
      // find first alive
      for(let k=0;k<s.players.length;k++){
        if(aliveAt(s, k)){ i = k; break; }
      }
    }
    setTurn(s, i);
  }

  // Strict: only current player may bid (no bidder-based exception)
  if(input.by !== s.turn.id) return s;
  ...
  return s;
            ...
            return s; // off-turn bid rejected once a turn id exists
          }
        }
        const nbid = { qty: Number(input.qty||input.payload?.qty), face: Number(input.face||input.payload?.face), by: input.by };
        if(!isValidRaise(s.bid, nbid, !!s.palifico)) return s;
      ...
      case 'dudo':{
        // Allow call only during bidding, with an existing bid, and by the current player.
        if(s.phase!=='bidding' || !s.bid) return s;
        // Self-heal missing turn (race)
        if(!s.turn || !s.turn.id){
          let i = (s.starter && Number.isInteger(s.starter.i)) ? s.starter.i : 0;
          if(!aliveAt(s,i)){ for(let k=0;k<s.players.length;k++){ if(aliveAt(s,k)){ i=k; break; } } }
          setTurn(s, i);
        }
        if(input.by !== s.turn.id) return s;
  ...
  Turns.init = function initTurnEngine(){
    console.log('[Turns.init] starting');
    try{ console.log('[Turns] engine active (host reducer)'); window.__USING_TURNS = true; }catch(e){}
    try{ console.log('[Turns] init'); }catch(e){}
    try{ window.__TURNS_ACTIVE = true; }catch(e){}
    try{
      if(!window || !window.firebase) return;
      ...
      if(isHost){ console.log('[Turns.init] host mode ON');
        ...
        inputsRef.limitToLast(1).on('child_added', snap=>{
          try{ console.log('[Turns] intent', snap.val()); }catch(e){}
          const ev = snap.val() || {};
          stateRef.once('value').then(ss=>{
            const cur = ss.val() || {};
            const next = reduce(cur, ev);
            // Mirror fields for legacy UI that still reads turnIndex/roundStarterIndex
            try{
              if (typeof next.turnIndex === 'undefined' && next.turn && Number.isInteger(next.turn.i)) next.turnIndex = next.turn.i;
              if (typeof next.roundStarterIndex === 'undefined' && next.starter && Number.isInteger(next.starter.i)) next.roundStarterIndex = next.starter.i;
            }catch(e){}

            try{ console.log('[Turns] reduce', ev.type, 'by', ev.by); }catch(e){}
