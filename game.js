...
const controls = $('#controls');
  ...
  const myTurn = hasTurnIndex ? (state.players[state.turnIndex].id === uid) : true; // if unknown, keep buttons usable
...
function startRoundHost(startTurnIndex=null){
  // --- robust init for new round ---
  try {
    console.log('[startRoundHost:init]', { startTurnIndex, players: (state.players||[]).map(p=>p && p.id) });
  } catch(e) {}
  if (!Number.isInteger(state.turnIndex)) {
    if (Number.isInteger(startTurnIndex)) {
      state.turnIndex = startTurnIndex;
    } else {
      const aliveIdx = (state.players||[]).map((p,i)=> p && p.dice>0 ? i : -1).filter(i=>i>=0);
      state.turnIndex = aliveIdx.length ? aliveIdx[0] : 0;
    }
  }
  if (!Number.isInteger(state.roundStarterIndex)) {
    state.roundStarterIndex = state.turnIndex;
  } else if (Number.isInteger(startTurnIndex)) {
    state.roundStarterIndex = startTurnIndex;
  }
...
// Strict host guard
const __cur = state.players && state.players[state.turnIndex];
if (!__cur || state.reveal || state.gameOver || __cur.id !== byId) { return; }
// Strict host guard
...
// Strict host guard
const __cur = state.players && state.players[state.turnIndex];
if (!__cur || state.reveal || state.gameOver || __cur.id !== callerId) { return; }
// Strict host guard
  const cur = state.players && state.players[state.turnIndex];
  if (!cur || state.reveal || state.gameOver || cur.id !== callerId) { return; }
