...
function renderControls(){
  // Keep controls ALWAYS visible. Host enforces legality; client shouldn't block clicks.
  const myTurn = state.players[state.turnIndex]?.id===uid;
  const revealOrOver = !!state.reveal || !!state.gameOver;
  const controls = $('#controls');
  if(controls) controls.classList.remove('hidden');

  // Don't hard-disable Bid/Dudo; allow intent to be sent even if slightly out-of-sync.
  const bidBtn=$('#btnBid'), dudoBtn=$('#btnDudo'), calzaBtn=$('#btnCalza');
  if(bidBtn){ bidBtn.disabled = false; }
  if(dudoBtn){ dudoBtn.disabled = false; }
  if(calzaBtn){
    calzaBtn.disabled = state.palifico; // hide/disable Calza in palifico rounds
    calzaBtn.classList.toggle('hidden', state.palifico);
  }

  // Keep first-bid face/qty guidance accurate
  setFirstBidControls();
}
...
document.getElementById('btnBid').addEventListener('click', ()=>{
  ...
  // Server-authoritative: always send the intent; host will accept only if it's the current player's turn.
...
document.getElementById('btnDudo').addEventListener('click', ()=>{
  ...
  // Server-authoritative: always send; host enforces turn order.
  const cur = state.players && state.players[state.turnIndex];
  const myTurnLocal = !!(cur && cur.id === uid) && !state.reveal && !state.gameOver;
  if(!myTurnLocal){ toast(`If it's not your turn, the host will ignore this action.`); }
