...
// ===== TURN UI ENFORCER (single source of truth on the client) =====
function computePerms(state, uid){
  const busy = !!(state && (state.reveal || state.gameOver));
  const hasTurn = !!(state && state.turn && state.turn.id);
  const isMyTurn = hasTurn && state.turn.id === uid;
  const starterCanOpen = !hasTurn && !state?.bid && !!(state && state.starter && state.starter.id===uid);
  const canBid   = !busy && (isMyTurn || starterCanOpen);
  const canDudo  = !busy && isMyTurn && !!state?.bid;
  const canCalza = !busy && isMyTurn && !!state?.bid && !state?.palifico;
  return { canBid, canDudo, canCalza };
}

function enforceControls(){
  try{
    const bidBtn   = document.getElementById('btnBid');
    const dudoBtn  = document.getElementById('btnDudo');
    const calzaBtn = document.getElementById('btnCalza');
    if(!bidBtn && !dudoBtn && !calzaBtn) return;
    const p = computePerms(window.state || {}, window.uid);
    if (bidBtn)   bidBtn.disabled   = !p.canBid;
    if (dudoBtn)  dudoBtn.disabled  = !p.canDudo;
    if (calzaBtn){
      calzaBtn.disabled = !p.canCalza;
      calzaBtn.classList.toggle('hidden', !!(window.state && window.state.palifico));
    }
  }catch(e){ console.warn('[enforceControls]', e); }
}
// Gentle heartbeat to avoid UI drift caused by other code
(function(){
  try{
    if (window.__controlsHeartbeat) return;
    window.__controlsHeartbeat = setInterval(enforceControls, 250);
  }catch(e){}
})();
