...
function renderControls(){
  ...
  // Always show controls (don’t hide the footer)
...
function startRoundHost(startTurnIndex=null){
  ...
  state.reveal=null;
  syncState();
  renderControls(); // 🔥 ensure buttons reset exactly like round 1
...
document.getElementById('btnBid').addEventListener('click', ()=>{
  ...
  if(isHost){ makeBidHost(uid, q, f); }
  else { inputsRef.push({type:'bid', qty:q, face:f, by:uid, ts: firebase.database.ServerValue.TIMESTAMP}); }
});
