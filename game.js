...
function renderControls(){
  ...
  const starterIdx = Number.isInteger(state.roundStarterIndex) ? state.roundStarterIndex : 0;
  const starterId = state.players && state.players[starterIdx]?.id;
  const myTurn = hasTurn ? (state.players[state.turnIndex].id === uid) : (uid === starterId);
...
function startRoundHost(startTurnIndex=null){
  // ---- Immediate starter publish (host -> Firebase) ----
  try {
    ensureState && ensureState();
  } catch(e){}
  let __starter = Number.isInteger(startTurnIndex) ? startTurnIndex : (Number.isInteger(state.roundStarterIndex) ? state.roundStarterIndex : null);
  if(!Number.isInteger(__starter)){
    const aliveIdx = (state.players||[]).map((p,i)=> (p && p.dice>0) ? i : -1).filter(i=>i>=0);
    __starter = aliveIdx.length ? aliveIdx[0] : 0;
  }
  state.turnIndex = __starter;
  state.roundStarterIndex = __starter;
  if(typeof state.round !== 'number') state.round = 1;
  state.bid = null; state.reveal = null; state.message = '';
  try {
    if (typeof stateRef !== 'undefined' && stateRef && typeof stateRef.update==='function') {
  ...
  state.turnIndex = starter;            // <-- set immediately (no later nulling)
  // Reset per-round fields
  state.bid = null;
  state.reveal = null;
  state.message = '';
  // If you roll dice here in your code, keep it; but do not touch turnIndex afterwards.
  snap('startRoundHost:after');
}
function makeBidHost(byId, qty, face){
  const cur = state.players && state.players[state.turnIndex];
  if (!cur || state.reveal || state.gameOver || cur.id !== byId) { return; }
...
function endOfRoundHost(loser, reason){  ensureState();
  state.roundStarterIndex = nextAliveFrom(state.roundStarterIndex||0);
...
function processInput(a){
  ...
  // SPECIAL CASE: first bid of a round (host turnIndex not set yet)
  if (a && a.type==='bid' && !Number.isInteger(state.turnIndex)) {
    const starterIdx = Number.isInteger(state.roundStarterIndex) ? state.roundStarterIndex : 0;
    const starterId  = state.players[starterIdx]?.id;
    if (a.by !== starterId) {
      return; // only the designated starter may place the first bid
    }
  ...
  // Normal strict guard path for all other actions
