...
// ---- Turn ownership helpers (single source of truth) ----
function normName(s){
  return String(s||'').toLowerCase().replace(/\s*\(you\)\s*$/, '').trim();
}
function myDisplayName(){
  try{
    const cfg = loadCfg();
    return cfg.youName || (document.getElementById('youName') && document.getElementById('youName').value) || window.myName || '';
  }catch(e){ return window.myName || ''; }
}
function amCurrentPlayer(){
  try{
    const cur = state.players && state.players[state.turnIndex];
    if(!cur || state.gameOver) return false;
    if(cur.id === uid) return true;
    return normName(cur.name) === normName(myDisplayName());
...
function renderControls(){
  const show = amCurrentPlayer() && !state.reveal && !state.gameOver;
...
try {
  window.state = state;
  window.uid = typeof uid !== 'undefined' ? uid : window.uid;
  window.amCurrentPlayer = amCurrentPlayer;
