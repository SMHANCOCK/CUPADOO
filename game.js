...
function renderControls(){
  const hasTurn = !!(state.turn && state.turn.id);
  const myTurn = newRound ? (!hasTurn || (state.turn && state.turn.id===uid)) : (state.turn && state.turn.id===uid);
...
if(!(window.Turns && typeof window.Turns.init==='function')){
  inputsRef.on('child_added', s=>{
    if(!isHost) return;
    const a = s.val();
    try { processInput(a); } catch(e){ console.warn('[legacy processInput error]', e); }
    try { s.ref.remove(); } catch(e){}
  });
}
/*END_LEGACY_HOST_INPUTS*/
...
function initLeaderboard(){
  try{
    ...
    // Show panel on menu only (if you have a menu screen flag, tie to it; otherwise show always)
    box.style.display = 'block';

    const ref = firebase.database().ref('leaderboard').orderByChild('wins').limitToLast(10);
    ref.on('value', snap => {
      const val = snap.val() || {};
      const rows = Object.entries(val).map(([id, row]) => ({ id, name: row.name || 'Player', wins: row.wins||0 }));
      // orderByChild ascending + limitToLast => reverse for descending
      ...
      rows.forEach((r, idx)=>{
        ...
        li.textContent = `${r.name} — ${r.wins} win${r.wins===1?'':'s'}`;
        list.appendChild(li);
      });
    });
  }catch(e){ console.warn('[initLeaderboard]', e); }
}

// call once on boot
try{ if(document.getElementById('leaderboard')) initLeaderboard(); }catch(e){}


// ===== Hijack legacy host mutators when Turn engine is active =====
(function(){
  try{
    if(!(window.Turns && typeof window.Turns.init==='function')) return;
    // Wrap only once
    if(window.__mutatorsHijacked) return;
    window.__mutatorsHijacked = true;

    const push = (obj)=>{ try{ inputsRef && inputsRef.push(Object.assign({ts: Date.now(), by: uid}, obj)); }catch(e){ console.warn(e); } };

    // Replace direct mutators to go through inputs
    if(typeof window.makeBidHost === 'function'){
      const _old = window.makeBidHost;
      window.makeBidHost = function(byId, qty, face){ push({type:'bid', qty, face}); };
    }
    if(typeof window.handleDudoHost === 'function'){
      const _old = window.handleDudoHost;
      window.handleDudoHost = function(){ push({type:'dudo'}); };
    }
    if(typeof window.handleCalzaHost === 'function'){
      const _old = window.handleCalzaHost;
      window.handleCalzaHost = function(){ push({type:'calza'}); };
    }
    if(typeof window.finishRevealAndResolveHost === 'function'){
      const _old = window.finishRevealAndResolveHost;
      window.finishRevealAndResolveHost = function(){ push({type:'continueReveal'}); };
    }
  }catch(e){ console.warn('[mutator hijack]', e); }
})();