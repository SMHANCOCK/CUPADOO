...
document.getElementById('btnBid').addEventListener('click', ()=>{
  ...
  const myName = (window.myName || (document.getElementById('name') && document.getElementById('name').value) || '').trim();
  const myTurn = !!(cur && (cur.id === uid || (myName && cur.name === myName))) && !state.reveal && !state.gameOver;
  if(!myTurn){ toast(`It's ${cur?cur.name:'another player'}'s turn`); return; }
...
/* === CUPADOO turn fix patch (round-agnostic) ===
   - Enable controls for current player based on player.id matching local ids
   - Do NOT block on 'reveal'; only on gameOver
   - Re-apply enable/disable every frame to beat legacy UI code
   - Click-guard only blocks when not canAct()
*/
(function(){
  function localIds(){
    const out = [];
    try{ if(window.uid) out.push(String(window.uid)); }catch(e){}
    try{ if(window.youId) out.push(String(window.youId)); }catch(e){}
    try{ if((window.state||{}).youId) out.push(String((window.state||{}).youId)); }catch(e){}
  ...
  function isCurrentTurn(){
    ...
    if(!cur) return false;
    const ids = localIds();
    // allow name match as last resort
    return ids.includes(String(cur.id)) || (window.youName && cur.name === window.youName);
  }
  function canAct(){
    const s = window.state || {};
    return isCurrentTurn() && !s.gameOver;
  }
  window.cupStatus = function(tag){
    const s = window.state || {};
    const cur = (s.players||[])[s.turnIndex];
    console.log('[CUP STATUS]', tag||'', {
      round:s.round, turnIndex:s.turnIndex,
      curId: cur && cur.id, curName: cur && cur.name,
      myIds: localIds(), myName: window.youName, canAct: canAct(),
      reveal: !!s.reveal, gameOver: !!s.gameOver, bid: s.bid||null
    });
  };
  ...
  function applyLock(){
    try{
      ...
      if(calza){
        calza.classList.toggle('hidden', !!s.palifico);
      }
    }catch(e){}
  }

  // Frame enforcement
  (function loop(){ applyLock(); requestAnimationFrame(loop); })();

  // Guard clicks from non-actors only
  document.addEventListener('click', function(ev){
    ...
    if(!canAct()){
      ...
      try{
        const s = window.state || {};
        const cur = (s.players||[])[s.turnIndex];
        const who = cur ? (cur.name || cur.id) : 'other player';
        if(typeof window.toast === 'function'){ window.toast(`It's ${who}'s turn`); }
      }catch(e){}
    }
  }, true);

  // Make MakeBid/Dudo/Calza trustworthy
  ['makeBid','onMakeBid','doMakeBid','handleMakeBid','dudo','onDudo','doDudo','handleDudo','calza','onCalza','doCalza','handleCalza']
    .forEach(function(fn){
      const orig = window[fn];
      if(typeof orig === 'function'){
        window[fn] = function(){
          if(!canAct()){ window.cupStatus('block-'+fn); return; }
...
/* v2 name/ID normalization hotfix */
(function(){
  function norm(x){ return String(x||'').toLowerCase().replace(/\s*\(you\)\s*$/,'').trim(); }
  // Override isCurrentTurn to allow normalized-name fallback if IDs are absent/mismatched
  const prevIsCurrentTurn = (typeof isCurrentTurn === 'function') ? isCurrentTurn : null;
  window.__cup_prevIsCurrentTurn = prevIsCurrentTurn;
  ...
  window.isCurrentTurn = function(){
    try{
      const s = window.state || {};
      const cur = (s.players||[])[s.turnIndex];
      if(!cur) return false;
      const ids = localIds();
      if (ids.includes(String(cur.id))) return true;
      // Normalized name fallback
      const myNames = [window.youName, window.displayName, (s.me && s.me.name)].map(norm);
      const curName = norm(cur.name);
      if (myNames.some(n => n && n === curName)) return true;
    }catch(e){}
    return false;
  };

  // Expose quick console probe
  window.cupWho = function(){
    const s = window.state||{};
    const cur = (s.players||[])[s.turnIndex];
    return {
      turnIndex:s.turnIndex, curId: cur?.id, curName: cur?.name,
      ids: localIds(), youName: window.youName, displayName: window.displayName,
      isCurrentTurn: window.isCurrentTurn()
    };
  };
})();
...
/* === On-screen debug overlay (tap-to-toggle) === */
(function(){
  function el(tag, cls){ const e=document.createElement(tag); if(cls) e.className=cls; return e; }
  function fmt(obj){ try{return JSON.stringify(obj,null,2);}catch(e){return String(obj);} }
  function who(){ try{return window.cupWho?window.cupWho():{};}catch(e){return {err:String(e)};} }
  function stat(tag){ try{ if(window.cupStatus) window.cupStatus(tag||'overlay'); }catch(e){} }

  function build(){
    if(document.getElementById('cup-debug')) return;
    const box = el('div'); box.id='cup-debug';
    box.style.cssText='position:fixed;z-index:99999;bottom:12px;left:12px;right:12px;max-height:45vh;overflow:auto;background:rgba(0,0,0,.85);color:#0ff;font:12px/1.4 monospace;border:1px solid #0ff;border-radius:8px;padding:10px;display:none;';
    const head = el('div'); head.textContent='CUPADOO DEBUG (tap to hide)'; head.style.cssText='font-weight:bold;margin-bottom:6px;color:#fff;';
    const pre = el('pre'); pre.id='cup-debug-pre'; pre.style.cssText='white-space:pre-wrap;';
  ...
  function show(tag){
    build();
    const pre = document.getElementById('cup-debug-pre');
    const s = window.state||{};
    const cur = (s.players||[])[s.turnIndex];
    const snap = {
      tag: tag||'',
      round: s.round, turnIndex: s.turnIndex,
      cur: cur?{id:cur.id, name:cur.name}:null,
      myIds: (function(){const a=[];try{if(window.uid)a.push(String(window.uid))}catch(e){}try{if(window.youId)a.push(String(window.youId))}catch(e){}try{if((window.state||{}).youId)a.push(String((window.state||{}).youId))}catch(e){};return a})(),
      myName: window.youName||window.displayName||null,
      canAct: (function(){try{return window.isCurrentTurn?window.isCurrentTurn():null}catch(e){return null}})(),
      reveal: !!s.reveal, gameOver: !!s.gameOver, bid: s.bid||null
    };
    pre.textContent = fmt(snap);
    const box = document.getElementById('cup-debug'); box.style.display='block';
    stat(tag||'overlay');
  }
  window.cupOverlay = { show };
  // Tap the "Version" label 5 times to toggle overlay
  document.addEventListener('click', function(e){
    const t = e.target;
    if(!t) return;
    const txt = (t.textContent||'').toLowerCase();
    if(txt.includes('version:')){
      window.__cupTapCount = (window.__cupTapCount||0)+1;
      if(window.__cupTapCount>=5){ window.__cupTapCount=0; show('tap'); }
      setTimeout(()=>{ window.__cupTapCount=0; },1200);
    }
  }, true);

  // Also show when we block an action
  const oldToast = window.toast;
  window.toast = function(msg){
    try{ if(String(msg||'').toLowerCase().includes("it's") && String(msg||'').toLowerCase().includes("turn")){ show('blocked'); } }catch(e){}
    if(typeof oldToast==='function') return oldToast.apply(this, arguments);
  };
})();
