...
// ---- Turn gating flags (UI-local, do not sync) ----
let __lastTurnId = null;
let __actedThisTurn = false;

function setActionButtonsEnabled(on){
  try{
    const ids=['btnBid','btnDudo','btnCalza'];
    for(const id of ids){
...
function renderControls(){
  try{
    ...
    if(controls) controls.classList.remove('hidden'); // keep visible always

    const myTurn = isMyTurn();
    const phaseOk = !state.reveal && !state.gameOver;
    const enable = !!(myTurn && phaseOk && !__actedThisTurn);

    setActionButtonsEnabled(enable);

    // Palifico hides/disables Calza
    const calzaBtn = document.getElementById('btnCalza');
    if(calzaBtn){
      calzaBtn.disabled = calzaBtn.disabled || !!state.palifico;
...
// reset local per-turn action flag when the active player changes
try {
  const curTurnId = (g && g.turn && g.turn.id) || (g.players && g.players[g.turnIndex] && g.players[g.turnIndex].id) || null;
  if(curTurnId !== __lastTurnId){
    __lastTurnId = curTurnId;
    __actedThisTurn = false;
  }
} catch(e){}
...
document.getElementById('btnBid').addEventListener('click', ()=>{
  // Guard: only act once per own turn
  if(!isMyTurn() || __actedThisTurn){ if(!isMyTurn()) toast(`It's not your turn.`); return; }
...
document.getElementById('btnDudo').addEventListener('click', ()=>{
  ...
  const myTurnLocal = !!(cur && cur.id === uid) && !state.reveal && !state.gameOver;
  if(!myTurnLocal){ toast(`If it's not your turn, the host will ignore this action.`); }
  if(isHost){ setLastChoice(uid,{type:'dudo'
  __actedThisTurn = true;
  setActionButtonsEnabled(false);
...
document.getElementById('btnCalza').addEventListener('click', ()=>{
  ...
  const myTurnLocal = !!(cur && cur.id === uid) && !state.reveal && !state.gameOver;
  if(!myTurnLocal){ toast(`If it's not your turn, the host will ignore this action.`); }
  if(isHost){ setLastChoice(uid,{type:'calza'
  __actedThisTurn = true;
  setActionButtonsEnabled(false);
