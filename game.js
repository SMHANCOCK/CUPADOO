...
function ensureState(){
  ...
  if(state.reveal === undefined) state.reveal = null;
...
function renderControls(){
  ...
  const hasTurn = Number.isInteger(state.turnIndex) && state.players && state.players[state.turnIndex];
  const myTurn = hasTurn ? (state.players[state.turnIndex].id === uid) : true;
  const busy = !!state.reveal || !!state.gameOver;
  const newRound = !state.bid && !busy; // no current bid yet

  const bidBtn  = $('#btnBid');
  const dudoBtn = $('#btnDudo');
  const calzaBtn= $('#btnCalza');

  // At the start of a round, always allow placing the very first bid.
  if (bidBtn)  bidBtn.disabled  = newRound ? false : (!myTurn || busy);
  if (dudoBtn) dudoBtn.disabled = !myTurn || busy; // never allowed for first action
  if (calzaBtn){
    calzaBtn.disabled = !myTurn || busy || state.palifico || newRound;
...
function startRoundHost(startTurnIndex=null){
  ...
  // Compute starter
  let starter = Number.isInteger(startTurnIndex) ? startTurnIndex : state.roundStarterIndex;
  if(!Number.isInteger(starter)){
    const aliveIdx = (state.players||[]).map((p,i)=> (p && p.dice>0) ? i : -1).filter(i=>i>=0);
    starter = aliveIdx.length ? aliveIdx[0] : 0;
  }
  ...
  // Publish immediately so clients have turnIndex
  try{
    if(stateRef && typeof stateRef.update==='function'){
      stateRef.update({ turnIndex: state.turnIndex, roundStarterIndex: state.roundStarterIndex, round: state.round, bid: state.bid, reveal: state.reveal, message: state.message });
    }
  }catch(e){}
...
function processInput(a){
  ...
  // First bid path: if turnIndex missing, set to bidder and accept
  if (a && a.type==='bid' && !Number.isInteger(state.turnIndex)) {
    let idx = state.players.findIndex(p => p && p.id === a.by);
    if (idx < 0) idx = 0;
    state.turnIndex = idx;
    if (typeof state.round !== 'number') state.round = 1;
    if (state.reveal == null) state.reveal = null;
    makeBidHost(a.by, a.qty, a.face);
    return;
  }
...
document.getElementById('btnBid').addEventListener('click', ()=>{
  ...
  // Do NOT block locally; always send intent, host validates.
  if(isHost){
    try{ makeBidHost(uid, q, f); }catch(e){ console.warn(e); }
  } else {
    try{ inputsRef.push({type:'bid', qty:q, face:f, by:uid, ts: Date.now()}); }catch(e){ console.warn(e); }
  }
...
document.getElementById('btnDudo').addEventListener('click', ()=>{
  ...
  if(!myTurnLocal){ toast(`If it's not your turn, the host will ignore this action.`); }
  if(isHost){
    try{ setLastChoice(uid,{type:'dudo'}); handleDudoHost(uid); syncState(); }catch(e){ console.warn(e); }
  } else {
    try{ inputsRef.push({type:'dudo', by:uid, ts: Date.now()}); }catch(e){ console.warn(e); }
  }

}); handleDudoHost(uid); syncState(); } else { inputsRef.push({type:'dudo', by:uid, ts: firebase.database.ServerValue.TIMESTAMP}); }
});
...
document.getElementById('btnCalza').addEventListener('click', ()=>{
  ...
  if(!myTurnLocal){ toast(`If it's not your turn, the host will ignore this action.`); }
  if(isHost){
    try{ setLastChoice(uid,{type:'calza'}); handleCalzaHost(uid); syncState(); }catch(e){ console.warn(e); }
  } else {
    try{ inputsRef.push({type:'calza', by:uid, ts: Date.now()}); }catch(e){ console.warn(e); }
  }

}); handleCalzaHost(uid); syncState(); } else { inputsRef.push({type:'calza', by:uid, ts: firebase.database.ServerValue.TIMESTAMP}); }
});
...
// ===== UI heartbeat: keep first-bid button enabled even if no render tick fired
(function firstBidHeartbeat(){
  try{
    setInterval(()=>{
      try{
        const bidBtn = document.getElementById('btnBid');
        if(!bidBtn) return;
        const busy = !!(window.state && (state.reveal || state.gameOver));
        const newRound = !!(window.state && !state.bid && !busy);
        if(newRound){
