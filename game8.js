...
function renderControls(){
  ...
  const hasTurnIndex = Number.isInteger(state.turnIndex) && state.players && state.players[state.turnIndex];
  const myTurn = hasTurnIndex ? (state.players[state.turnIndex].id === uid) : true;
  const busy = !!state.reveal || !!state.gameOver;

  const bidBtn  = $('#btnBid');
  const dudoBtn = $('#btnDudo');
  const calzaBtn= $('#btnCalza');

  if (bidBtn)  bidBtn.disabled  = !myTurn || busy;
  if (dudoBtn) dudoBtn.disabled = !myTurn || busy;

  if (calzaBtn){
    calzaBtn.disabled = !myTurn || busy || state.palifico;
...
function startRoundHost(startTurnIndex=null){
  // --- Robust Round Init (prevents undefined round/turnIndex) ---
  try { console.log('[SRH:init]', {startTurnIndex, players:(state.players||[]).map(p=>p&&p.id)}); } catch(e){}
  if (!Array.isArray(state.players)) { state.players = []; }
  const __aliveIdx = (state.players||[]).map((p,i)=> (p && p.dice>0) ? i : -1).filter(i=>i>=0);
  if (!Number.isInteger(state.turnIndex)) {
    if (Number.isInteger(startTurnIndex)) state.turnIndex = startTurnIndex;
    else state.turnIndex = (__aliveIdx.length ? __aliveIdx[0] : 0);
  }
  if (!Number.isInteger(state.roundStarterIndex)) {
    state.roundStarterIndex = state.turnIndex;
  }
  if (typeof state.round !== 'number') {
    state.round = 1;
  }
  if (state.reveal == null) state.reveal = null;
  if (state.bid === undefined) state.bid = null;
  if (state.message === undefined) state.message = '';
  // ensure UI gets a tick with initialized fields
...
function processInput(a){
  // Robust host-side handling of first action at round start
  if (!Array.isArray(state.players)) state.players = [];
  if (!Number.isInteger(state.turnIndex) && a && a.type === 'bid') {
    // If host hasn't committed a turnIndex yet, assign it to the bidder.
    const idx = state.players.findIndex(p => p && p.id === a.by);
    if (idx >= 0) state.turnIndex = idx;
  }
  if (typeof state.round !== 'number') state.round = 1;
  if (state.reveal == null) state.reveal = null;

  // Strict guard (except allow host to continue reveal)
  const curId = state.players[state.turnIndex]?.id;
  if (a.type !== 'continueReveal') {
    if (!curId || a.by !== curId || state.reveal || state.gameOver) {
      return;
    }
  } else {
    if (!isHost) return;
...
function __stringifyArg(a){
  try{
    ...
    try { return String(a); } catch(_){ return '[unprintable]'; }
  }
}
function initDebugLogging(){
  try{
    ...
    if(!roomCode) return;
    __logsRef = firebase.database().ref(`rooms/${roomCode}/logs`);
    // Patch local console to also push to Firebase
    ['log','warn','error'].forEach(level=>{
      const orig = console[level];
      console[level] = function(...args){
        try{
          const msg = args.map(__stringifyArg).join(' ');
          __logsRef.push({ by: uid || '?', level, msg, ts: Date.now() });
        }catch(e){ /* ignore */ }
        try{ orig.apply(console, args); }catch(e){ /* ignore */ }
      };
    });
    // If host, also subscribe to remote logs and echo them to console
    if(isHost){
      __logsRef.limitToLast(300).on('child_added', snap=>{
        const ev = snap.val();
        if(!ev) return;
        if(ev.by && uid && ev.by === uid) return; // don't echo our own
        const prefix = `[REMOTE ${ev.by||'?'} ${ev.level}]`;
        try{
          (console[ev.level] || console.log)(prefix, ev.msg);
        }catch(e){
          try{ console.log(prefix, ev.msg); }catch(_){}
        }
      });
    }
  }catch(e){ /* ignore */ }
...
// ===== Lightweight on-screen debug (toggle with backtick ` ) =====
(function(){
  let panel=null, on=false;
  function show(m){
    if(!panel){
      panel=document.createElement('pre');
      panel.id='cupadooDebug';
      panel.style.cssText='position:fixed;bottom:8px;left:8px;right:8px;max-height:40vh;overflow:auto;background:rgba(0,0,0,0.7);color:#0f0;padding:8px;font:12px/1.2 monospace;z-index:99999;border:1px solid #0f0;border-radius:8px;display:none;';
  ...
  window.__cupadooDebug = {
    log:(tag,obj)=>show(tag+' '+JSON.stringify(obj))
  };
  window.addEventListener('keydown', (e)=>{
    if(e.key==='`'){ on=!on; if(panel) panel.style.display=on?'block':'none'; }
  });
  // hook snap to also post to overlay
  const oldSnap = window.snap;
  window.snap = (tag='')=>{
    try{
      const s = window.state||{};
      const ps = s.players||[];
      const cur = ps[s.turnIndex];
      const obj = {round:s.round, turnIndex:s.turnIndex, cur: cur?{id:cur.id,name:cur.name}:null, reveal:!!s.reveal, gameOver:!!s.gameOver, bid:s.bid?{qty:s.bid.qty,face:s.bid.face,by:s.bid.by}:null};
      __cupadooDebug.log('[SNAP '+tag+']', obj);
      if(typeof oldSnap==='function'){ oldSnap(tag); }
    }catch(e){}
  };
})();
